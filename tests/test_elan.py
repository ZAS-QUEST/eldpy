...
def test_wan(capsys):
    ...
    assert x[504:676] == '"a3","nàà gà lé éé klā tàlí yá̰","nàà\tgà\tlé\téé\tklā\ttàlí\tyá̰","1sg+cop\tgo\tprog\t3sg:indep\tbehind\tnow\tthere","I will [explain] that now.","","WORD_ALIGNED"'
...
def test_yaminawa_p(capsys):
    ...
    assert x[204:527] == '"ann11_flexid_7a42f816-ed01-4f28-9608-e83491ddf21b","Pashpiki askadikia, askapaudikia","pashpi\t=ki\taska\t-di\t=kia\taska\t-pau\t-di\t=kia","masculine.name\t=LAT\tdo.like.this.DIST\tREM.PST\t=EVID.REP\tdo.like.this.DIST\tIMPRF.PST.REM\tREM.PST\t=EVID.REP","They say Pashpi was like this, this is what they say happened.","","WORD_ALIGNED"'
...
def test_muyu(capsys):
    ...
    assert x[378:525] == '"a153","Ege olalane keman eyen.","#\tege\tolal-an-e\tkem-an\teyen","#\tDEM\ttalk-IRR-SM\tAUX-1SG\tthis.is","This is what I want to tell.","","WORD_ALIGNED"'
...
def test_mbat(capsys):
    ...
    assert x[636:841] == '"ann112_flexid_889cddd3-69e6-4585-88a6-5dfd0330530c","yan ɓɪlɪm munɓalya.","ya\t=n\tɓɪl\t-ɪm\tmʊn\tɓal\t=ya","3PL\t=FOC\tgive birth\tPFT\tchild\tkind\t=3PL","they have given birth to their children.","","WORD_ALIGNED"'
...
def test_windhoek(capsys):
    ef = ElanFile("KK-Windhoek-20230722-5a-C_mod.eaf", "www") # totoli_test.eaf has two speakers
    ef.populate(translationcandidates=['fte'])
    x = ef.get_cldfs()
    # with capsys.disabled():
    #     print(repr(x[498:645]))
    assert x[498:645] == '"a3938","Ashais ai ta gege ǃnae","Ashai\t-s\tai\t=ta\tge\tge\tǃnae","Ashais\t-3F.SG\ton\t=1SG.SBJ\tDECL\tPST\tbe_born","I was born in Ashais","","WORD_ALIGNED"'
